...
const templateSelect = (data = [], defaultText = "Выберите из списка") => {
  ...
  <button type="button" class="select__trigger" data-select="trigger"> <i class="bi bi-globe"></i> <span class="select__text">${defaultText}</span>
...
class CustomSelect {
  ...
  _changeItem(item) {
    if (!item.classList.contains("select__item_selected")) {
      ...
      this._$trigger.innerHTML = `<i class="bi bi-globe"></i> <span class="select__text">${item.textContent}</span>`;
